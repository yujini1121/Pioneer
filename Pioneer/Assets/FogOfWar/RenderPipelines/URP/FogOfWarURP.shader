Shader "Hidden/FogOfWarURP"
{
	...
	#pragma multi_compile STYLE_DEFAULT STYLE_SDF
    ...
    uniform float _PartialFogAmount;
    uniform float _OutsideFogStrength;
	uniform float2 _FogHeightRange;
	...
	Varyings Vert(Attributes input)
	{
		...
		output.texcoord = output.positionCS * 0.5 + 0.5;
	...
	float3 ComputeViewSpacePosition(float2 texcoord, float3 ray, out float rawdepth)
	{
		...
		// Android is doing something weird so reverse what it does here
	...
	float2 WorldPositionToFogPosition(float3 worldpos, out float height)
	{
		#ifdef PLANE_XY
	...
	float GetFogAmount(float2 fogpos, float rawdepth)
	{
		...
		// combine visible and partial
		float3 fogcombined = SAMPLE_TEXTURE2D(_FogTex, sampler_FogTex, fogpos).rgb;
		float visiblefog = fogcombined.r * fogcombined.b;
		float partialfog = fogcombined.g;
	
		#if STYLE_SDF
			float fog;
			if (visiblefog < 0.5)
				fog = 0;
			else if (partialfog < 0.5)
				fog = 1 - _PartialFogAmount;
			else
				fog = 1;
		#else// STYLE_DEFAULT
			float fog = lerp(visiblefog, partialfog * fogcombined.b, _PartialFogAmount);
		#endif

		// if outside map, use the outside fog color
		fog = lerp(fog, _OutsideFogStrength, isoutsidemap);
	...
	FogData GetFogData(float2 uv, float3 interpolatedray)
	{
		...
		fogdata.mapPosition = WorldPositionToFogPosition(fogdata.worldPosition, fogheight);
		...
		if (fogheight < _FogHeightRange.x || fogheight > _FogHeightRange.y)
			fogdata.fogAmount = 1;
