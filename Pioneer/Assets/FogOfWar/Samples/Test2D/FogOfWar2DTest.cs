    ...
    public class FogOfWar2DTest : MonoBehaviour
    {
        ...
        class Chunk
        {
            ...
            public Chunk(FogOfWar2DTest test, Vector3Int chunkpos, TerrainSet[] terrainsets)
            {
                ...
                parent.position = _test._chunkManager.ChunkCornerToWorldPositionCorrect(chunkPosition);
        ...
        {
            Vector3Int pos3 = _chunkManager.WorldPositionToChunk(pos);
            return new Vector2Int(pos3.x, pos3.y);
        }
        ...
        void UpdateChunks()
        {
            ...
            Vector2 chunkcenteroffset = new Vector2(0.5f, 0.5f);
            
            Vector2Int minchunk = PositionToChunk(camerapos - orthosize + chunkcenteroffset);
            Vector2Int maxchunk = PositionToChunk(camerapos + orthosize + chunkcenteroffset);
        ...
        void UpdatePlayerMovement()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space))
                FogOfWarTeam.GetTeam(0).SetAll(FogOfWarValueType.Partial, 255);
            ...
            if (Vector2.Distance(playerpos, _targetPosition) < 0.01f)
            {
                ...
                {
                    Vector2 touchpos = (Vector2)Input.mousePosition - new Vector2(Screen.width / 2, Screen.height / 2);
                ...
                {
                    RaycastHit2D hit = Physics2D.Raycast(_targetPosition + new Vector2(0.5f, 0.5f), dir, 1);
