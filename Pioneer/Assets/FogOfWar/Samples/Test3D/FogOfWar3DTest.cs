...
namespace FoW
{
    ...
    public class FogOfWar3DTest : MonoBehaviour
    {
        ...
        public FogOfWarTextureSource clearFog;
        ...
        void Update()
        {
            ...
            // select unit
            RaycastHit hit;
            if (Input.GetKeyDown(KeyCode.Mouse0) && Physics.Raycast(mainCamera.ScreenPointToRay(Input.mousePosition), out hit))
            {
            ...
            if (_selectedUnit != null && Input.GetKeyDown(KeyCode.Mouse1) && Physics.Raycast(mainCamera.ScreenPointToRay(Input.mousePosition), out selecthit))
            {
            ...
            if (Input.GetKeyDown(KeyCode.Mouse2) && Physics.Raycast(mainCamera.ScreenPointToRay(Input.mousePosition), out hit))
                _team.SetFog(FogOfWarValueType.Partial, new Bounds(hit.point, Vector3.zero), 0);
