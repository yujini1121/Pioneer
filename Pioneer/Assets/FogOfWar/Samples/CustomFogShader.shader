Shader "FoW/CustomFogShader"
{
	Properties
	{
		...
		_PartialFogAmount ("Partial Fog Amount", Range(0, 1)) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
			...
			float GetFogAmountAtPosition(float2 fogpos)
			{
				...
				float2 fogPixelData = tex2D(_FogTex, fogpos).rg;
				float fog = saturate(fogPixelData.g + fogPixelData.r * _PartialFogAmount);
